...
geom_line(aes(x = Ndate, y = Namount,group=1), linetype = "solid",color = "coral1") +
  labs(title = "GRU Monthly Bill",
       subtitle = paste("Bill by Month:",min(df$Ndate),"to",max(df$Ndate), sep=" "),
       x = "Date", y = "$ Amount")
...
gru_log_bill %>%    
  ...
  geom_density() +
  geom_rug(alpha = 0.5) +
  labs(title = "Histogram: GRU Monthly Bill (Log)",
       subtitle = paste("Date Range:",min(df$Ndate),"to",max(df$Ndate), sep=" "),
       x = "log_Bill", y = "Frequency")
...
dist_Amount <- dist_log_bills %>% 
  exp() %>% 
  quantile(probs = probs, na.rm = TRUE) %>% 
  round(2) 
 
print("Proability distribution of bill (50% most likely): ")
dist_Amount 


mean_log_bills <- mean(dist_log_bills, na.rm = FALSE)
sd_log_bills <- sd(dist_log_bills, na.rm = FALSE)
...
meanBill <- mean_log_bills %>% exp() %>% round(2)

sdBill <- sd_log_bills %>% exp() %>% round(2)

paste("Mean Bill",meanBill,sep = " ") %>% print()
paste("SD",sdBill, sep=" ") %>% print()
